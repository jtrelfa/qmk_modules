...
static int                pom_total_cycle_duration   = POM_PREPARE_FLASH_DURATION + POM_WORKING_DURATION + POM_DONE_WORKING_FLASH_DURATION + POM_RESTING_DURATION;
...
enum pom_led_state pom_get_current_state(uint32_t elapsed_time) {
    ...
    } else if (cycle_position < (POM_PREPARE_FLASH_DURATION + POM_WORKING_DURATION)) {
        return POM_LED_STATE_WORKING;
    } else if (cycle_position < (POM_PREPARE_FLASH_DURATION + POM_WORKING_DURATION + POM_DONE_WORKING_FLASH_DURATION)) {
...
bool rgb_matrix_indicators_pomodoro(void) {
    ...
    if (pom_current_led_state == POM_LED_STATE_PREPARE_FLASHING || pom_current_led_state == POM_LED_STATE_DONE_WORKING_FLASHING) {
